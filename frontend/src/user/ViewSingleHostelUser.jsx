...
const ViewSingleHostelUser = ({acceptReject}) => {
  ...
  const getHostelDetails = async () => {
    try {
      ...
      const res= await AxiosInstance.get(`users/hostel/${hostelId}`);
      console.log(res);
      setRooms(res.data.rooms);
      setFoodPackages(res.data.foods);
    } catch (error) {
      console.log(error)
    }
  }
...
:
            ...
            <h3 className="mt-10 p-5 text-lg font-bold text-purple-900">
             ...
             <div className=" bg-amber-50 p-3" key={index} >
                <img
                  className="w-full my-4 h-96 "
src={`${url}uploads/Hostel/Profiles/${pic}`}
                  // src="https://franchisehub.ca/wp-content/uploads/2024/01/REV-Zero-Royalty.jpg"
             ...
             <div className="" key={index}>
              ...
              {/* <div>
                ...
                />
                          ...
                          <p className='mt-5'>Number of Beds: {item.Number_of_Beds}</p>
                          <p>Rent : {item.Rent} Rs. </p>
                          <p>
                          facilties: {item.facilties}
                          </p>
                        </article>
                      </details>
                    </li>
 ...
 &&
                  foodPackages.map((item, index) => (
                    <li>
                      <details className="group">
                        ...
                        <article className="px-4 pb-4">
                          <p>
                          Meal_Timings: {item.Meal_Timings}
                          </p>
                          <p>
                          Type_of_Meal: {item.Type_of_Meal}
                          </p>
                          
                          <p>Package_Price : {item.Package_Price} Rs. </p>
                          <p>{item.Description}</p>
                        </article>
                      </details>
                    </li>
       ...
       <div className=" border-t flex justify-center p-10">
          <button className=" bg-gray-800 w-3/4 text-white rounded h-10  hover:bg-gray-500 " 
          onClick={
            ()=>navigateTo(`/user/make-payment/${hostelId}`, {state:{room: rooms, food:foodPackages}})
          }
          >
            Book Hostel 
          </button>
...
export default ViewSingleHostelUser